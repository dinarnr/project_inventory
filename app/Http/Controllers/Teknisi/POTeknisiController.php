<?php

namespace App\Http\Controllers\Teknisi;

use App\Http\Controllers\Controller;
use App\Models\DetailPO;
use App\Models\Instansi;
use App\Models\PO;
use App\Models\Profil;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class POTeknisiController extends Controller
{
    public function po()
    {
        $po = PO::all()->where('status','2');
        return view('teknisi/po/po', compact('po'));
    }

    public function detailpo($no_PO)
    {  $data_detail = DetailPO::where('no_PO', $no_PO)->get();
        $profil = Profil::all();
        $data_po = PO::where('no_PO', $no_PO)->get();
        $tanggal = Carbon::now();
        $total = DetailPO::where('no_PO', $no_PO)->sum('amount');
        $nama_instansi = PO::where('no_PO', $no_PO)->pluck('kode_instansi');
        $user = Auth::user();
        $instansi = Instansi::where('kode_instansi', $nama_instansi)->get();
        return view('teknisi/po/detailpo', compact('data_po', 'data_detail', 'user', 'profil', 'instansi', ));
    }
}
